document.addEventListener('DOMContentLoaded', () => {
  ...
  function showTab(tabName) {
      ...
      document.querySelectorAll('.tab-button').forEach(button => {
       ...
       if (tabName === 'settings') loadMigrations();
       ...
       try {
          ...
          if (accounts.length === 0) {
              ...
              accounts.forEach(acc => {
                  const row = `
                      <tr>
                          <td class="px-6 py-4 whitespace-nowrap">${acc.cus_name}</td>
                          <td class="px-6 py-4 whitespace-nowrap">${acc.cus_cif_no}</td>
                          <td class="px-6 py-4 whitespace-nowrap">${acc.account_no}</td>
                          <td class="px-6 py-4 whitespace-nowrap">${acc.account_currency}</td>
                          <td class="px-6 py-4 whitespace-nowrap">
                          ...
                          <td class="px-6 py-4 whitespace-nowrap text-right">${new Intl.NumberFormat('en-ZA', { style: 'currency', currency: acc.account_currency }).format(acc.curr_account_balance)}</td>
                      </tr>
                  `;
